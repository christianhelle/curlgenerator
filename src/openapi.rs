...
fn parse_openapi(content: &str) -> Result<OpenAPI> {
    ...
    // Try parsing with sw4rm-rs (handles both v2.0 and v3.x)
    let sw4rm_spec = if let Ok(spec) = serde_json::from_str::<Spec>(content) {
        Some(spec)
    } else if let Ok(spec) = serde_yaml::from_str::<Spec>(content) {
        Some(spec)
    } else {
        None
    };
    ...
    if let Some(spec) = sw4rm_spec {
        // Check if it's a v2.0 spec that needs conversion
        if spec.spec_version.starts_with("2.") {
            return convert_spec_to_openapi_v3(&spec);
        }
        // For v3.x specs parsed by sw4rm-rs, try to convert to openapiv3
        return convert_spec_to_openapi_v3(&spec);
    }
...
fn convert_parameter_v2_to_v3(param: &mut serde_json::Value) {
    if let Some(param_obj) = param.as_object_mut() {
        ...
        if param_obj.contains_key("type") && !param_obj.contains_key("schema") {
