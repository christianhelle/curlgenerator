...
download_and_install() {
  local platform="$1"
  local version="$2"
  local archive_name="curlgenerator-${version}-${platform}.tar.gz"
...
show_usage() {
  cat <<EOF
cURL Request Generator installation script

Usage: $0 [OPTIONS]

Options:
  -d, --dir DIR          Set the installation directory (default: ~/.local/bin)
...
Examples:
  ...
  curl -fsSL https://christianhelle.com/curlgenerator/install | INSTALL_DIR=\$HOME/.local/bin bash
  curl -fsSL https://christianhelle.com/curlgenerator/install | bash -s -- --version 0.4.1
EOF
}
...
main() {
  ...
  log_info "Starting cURL Request Generator installation..."
  check_dependencies
