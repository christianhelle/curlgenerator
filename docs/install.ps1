...
$GitHubRepo = "christianhelle/curlgenerator"
$BinaryName = "curlgenerator.exe"
$DocumentationUrl = "https://christianhelle.com/curlgenerator/"
...
{
  Write-Host ""
  Write-Host "cURL Request Generator installation script for Windows" -ForegroundColor Blue
  Write-Host ""
  Write-Host "Usage:" -ForegroundColor Yellow
  Write-Host "  irm https://christianhelle.com/curlgenerator/install.ps1 | iex" -ForegroundColor White
  Write-Host ""
...
{
  $candidates = @(
    "$env:LOCALAPPDATA\Programs\curlgenerator",
  ...
  return "$env:LOCALAPPDATA\Programs\curlgenerator"
}
...
function Get-ArchiveName
{
  ...
  switch ($architecture)
  {
    ...
    {
      return "curlgenerator-$ResolvedVersion-win-x64.zip"
    }
...
function Install-CurlGenerator
{
  param(
  ...
  $tempDir = Join-Path $env:TEMP "curlgenerator-install-$(Get-Random)"
...
function Main
{
  ...
  Write-Info "Starting cURL Request Generator installation for Windows..."
  ...
  $binaryPath = Install-CurlGenerator -ResolvedVersion $resolvedVersion -TargetDir $InstallDir
